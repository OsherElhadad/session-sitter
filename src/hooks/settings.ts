 ...
 */
export const PLUGIN_ENV: Readonly<Record<string, string>> = {
...
/** One setting, as resolved, with the command that changes it. */
export interface SettingRow {
  /** The field name {@link PluginSettings} uses, which is what the docs and the code both call it. */
  key: string;
  /** The resolved value, or null when nothing is set — which prints as "not set", never as a zero. */
  value: string | null;
  /** The variable that sets it. */
  env: string;
  /** The exact line a terminal types. This is the write path, and it is deliberately not a form. */
  command: string;
}
 ...
 */
export function settingRows(s: PluginSettings): SettingRow[] {
  const shown: Record<string, string | number | boolean | null> = {
    mode: s.mode,
  ...
  return Object.keys(PLUGIN_ENV).map(key => {
    const value = shown[key];
    const env = PLUGIN_ENV[key];
    return {
      key,
