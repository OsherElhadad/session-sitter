...
export function asksDir(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(dataDir(env), 'asks');
}

export function askPath(askId: string, env?: NodeJS.ProcessEnv): string {
  return path.join(asksDir(env), `${askId}.json`);
}

export function verdictPath(askId: string, env?: NodeJS.ProcessEnv): string {
...
export function waitSeconds(raw: string | undefined): number {
  if (raw === undefined || raw.trim() === '') { return DEFAULT_WAIT_SECONDS; }
  const n = Number(raw.trim());
  if (!Number.isFinite(n) || n < 1) { return DEFAULT_WAIT_SECONDS; }
  return Math.min(Math.floor(n), MAX_WAIT_SECONDS);
}

/** A question put to a human about one tool call. */
export interface Ask {
  askId: string;
  /** ISO instant the ask was written. */
  at: string;
  /** ISO instant after which no answer is accepted. */
  deadline: string;
  sessionId: string;
  cwd: string;
  host: string;
  tool: string;
  /** Redacted and truncated by the caller — never a raw tool input. */
  inputSummary: string;
  /** Why the ladder could not decide, in a sentence a human can act on. */
  reason: string;
  /** The hook process, so a stale ask can be recognised as abandoned. */
  pid: number;
}

/** A human's answer to one ask. */
export interface Verdict {
  askId: string;
  decision: 'allow' | 'deny';
  /** Who answered, as the channel identifies them. Never invented. */
  by: string;
  /** What they said, when they said anything beyond the decision itself. */
  text: string;
  at: string;
}

function isAsk(value: unknown): value is Ask {
  const a = value as Ask;
...
export async function writeAsk(ask: Ask, env?: NodeJS.ProcessEnv): Promise<void> {
...
export async function readAsk(file: string): Promise<Ask | null> {
  try {
...
export async function writeVerdict(verdict: Verdict, env?: NodeJS.ProcessEnv): Promise<void> {
...
export async function readVerdict(file: string): Promise<Verdict | null> {
  try {
    const parsed = JSON.parse(await fs.promises.readFile(file, 'utf8')) as unknown;
...
export async function pendingAsks(
  now: Date, env?: NodeJS.ProcessEnv,
): Promise<Ask[]> {
  ...
  for (const file of files.sort()) {
    ...
    if (Date.parse(ask.deadline) <= now.getTime()) { continue; }
    if (await readVerdict(verdictPath(ask.askId, env)) !== null) { continue; }
    out.push(ask);
  }
  return out;
}
 ...
 */
export async function sweepAsks(
  now: Date, graceMs: number, env?: NodeJS.ProcessEnv,
): Promise<number> {
  const dir = asksDir(env);
  let files: string[];
  try {
  ...
  for (const file of files) {
    ...
    if (file.endsWith('.verdict.json')) { continue; } // removed with its ask
    const ask = await readAsk(full);
    if (ask === null) { continue; }
    if (now.getTime() - Date.parse(ask.deadline) <= graceMs) { continue; }
    try { await fs.promises.unlink(full); removed++; } catch { /* already gone */ }
    try { await fs.promises.unlink(verdictPath(ask.askId, env)); } catch { /* none written */ }
  }
  return removed;
}
...
export interface AskOutcome {
  /** The human's answer, or null when the deadline passed with none. */
  verdict: Verdict | null;
  /** How long the hook actually waited, milliseconds — recorded, never estimated. */
  waitedMs: number;
}
 ...
 */
export async function askHuman(opts: AskHumanOptions): Promise<AskOutcome> {
  ...
  for (;;) {
    ...
    if (verdict !== null) { return { verdict, waitedMs: now() - startedAt }; }
    if (now() >= deadline) { return { verdict: null, waitedMs: now() - startedAt }; }
 ...
 */
export function renderAsk(ask: Ask): string {
