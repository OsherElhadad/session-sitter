...
import { DecisionRecord, appendJsonl } from '../audit/trail';
import { dataDir, decisionsPath } from './paths';
...
/** Top-level `decision`, per the hooks reference — not a `hookSpecificOutput`. */
...
export const GUARD_CITATION = 'built-in §config-guard';
 ...
 */
export const MODE_RANK: Readonly<Record<string, number>> = {
  plan: 0,
  default: 1,
  manual: 1,
  acceptEdits: 2,
...
function rank(mode: string | null): number {
  if (mode === null) { return MODE_RANK.default; }
  const known = MODE_RANK[mode];
  return known === undefined ? UNKNOWN_MODE_RANK : known;
}

/** Just the parts of a settings file that decide what the agent may do. */
...
export function permissionShape(settings: unknown): PermissionShape {
 ...
 */
export function widenings(before: PermissionShape, next: PermissionShape): string[] {
  const found: string[] = [];
  const had = new Set(before.allow);
  for (const rule of next.allow) {
    if (!had.has(rule)) { found.push(`permissions.allow gained "${rule}"`); }
  }
  const keeps = new Set(next.deny);
  for (const rule of before.deny) {
    if (!keeps.has(rule)) { found.push(`permissions.deny lost "${rule}"`); }
  }
...
export function snapshotPath(source: string, filePath: string, env?: NodeJS.ProcessEnv): string {
  const key = createHash('sha256').update(`${source} ${filePath}`, 'utf8').digest('hex').slice(0, 16);
  return path.join(dataDir(env), 'config-snapshots', `${key}.json`);
}

function readSnapshot(file: string): PermissionShape | null {
  try {
...
function writeSnapshot(file: string, shape: PermissionShape): void {
  try {
...
const SETTINGS_SOURCES: ReadonlySet<string> =
  new Set(['user_settings', 'project_settings', 'local_settings']);

function record(input: ConfigChangeInput, blocked: boolean, note: string, started: number): void {
...
export async function handle(rawInput: HookInput): Promise<ConfigChangeOutput> {
  const started = Date.now();
  const input = rawInput as ConfigChangeInput;
  ...
  if (widened.length === 0) {
    writeSnapshot(snapshot, next);
    record(input, false,
      `allowed — ${source} changed without widening what the agent may do`, started);
    return {};
  }
  ...
  record(input, true,
    `blocked — ${source} widened the agent's own permissions: ${widened.join('; ')}`, started);
  return {
    decision: 'block',
