...
export interface ShellSplit {
  /** Every command the line runs. Meaningless unless `confident` — ignore it when false. */
  commands: string[];
  /** False when the line could not be split with certainty. The caller must not treat it as safe. */
  confident: boolean;
  /** Why certainty was lost, for the decision message and the audit note. Null when confident. */
  reason: string | null;
}
...
interface Fail { reason: string | null }

/** Record the first loss of certainty. Only the first matters — the scan stops either way. */
function lose(fail: Fail, reason: string): void {
  if (fail.reason === null) { fail.reason = reason; }
}
...
function substitution(
  src: string, open: number, depth: number, out: string[], fail: Fail,
): number | null {
  if (depth >= MAX_DEPTH) { lose(fail, `substitution nested deeper than ${MAX_DEPTH}`); return null; }
  const close = matchParen(src, open);
  if (close < 0) { lose(fail, 'unterminated command substitution'); return null; }
...
function backtick(src: string, at: number, depth: number, out: string[], fail: Fail): number | null {
  if (depth >= MAX_DEPTH) { lose(fail, `substitution nested deeper than ${MAX_DEPTH}`); return null; }
  let i = at + 1;
  while (i < src.length) {
...
function doubleQuoted(
  src: string, at: number, depth: number, out: string[], fail: Fail,
  ...
  while (i < src.length) {
    ...
    if (c === '"') { return { text: `${text}"`, next: i + 1 }; }
    if (c === '`') {
      const next = backtick(src, i, depth, out, fail);
      if (next === null) { return null; }
      text += ' '; i = next; continue;
    }
    if (c === '$' && src[i + 1] === '(') {
      if (src[i + 2] === '(') { lose(fail, 'arithmetic expansion $(( ))'); return null; }
...
function scan(src: string, depth: number, out: string[], fail: Fail): void {
  let cur = '';
  let i = 0;
  const push = (): void => { const t = clean(cur); if (t) { out.push(t); } cur = ''; };

  while (i < src.length) {
    if (fail.reason !== null) { return; }
    const c = src[i];
    ...
    if (c === "'") {
      ...
      if (end < 0) { lose(fail, 'unbalanced single quote'); return; }
      cur += src.slice(i, end + 1); i = end + 1; continue;
    }

    if (c === '"') {
      const span = doubleQuoted(src, i, depth, out, fail);
      if (span === null) { return; }
      cur += span.text; i = span.next; continue;
    }
    ...
    if (c === '$' && src[i + 1] === '(') {
      ...
      if (src[i + 2] === '(') { lose(fail, 'arithmetic expansion $(( ))'); return; }
      const next = substitution(src, i + 1, depth, out, fail);
      if (next === null) { return; }
      cur += ' '; i = next; continue;
    }
    ...
    if (op !== undefined) { push(); i += op.length; continue; }

    cur += c; i++;
  }
  if (fail.reason === null) { push(); }
}
...
export function splitShellCommand(command: string): ShellSplit {
  ...
  const fail: Fail = { reason: null };
  scan(command, 0, commands, fail);
  if (fail.reason !== null) { return { commands, confident: false, reason: fail.reason }; }
