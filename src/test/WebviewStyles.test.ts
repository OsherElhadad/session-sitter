...
describe('webview styles.css: the panel follows the theme', () => {
  ...
  it('draws separators and shadows with the widget tokens', () => {
    ...
    // `.status-finished` is excluded on purpose: its box-shadow draws the ring around the dot, so
    // it is part of the marker's shape rather than a shadow the theme should be recolouring.
    const shadows = (css.match(/box-shadow:[^;]+;/g) ?? [])
      .filter(rule => !rule.includes('--vscode-charts-green'));
...
describe('webview: keyboard and screen-reader access', () => {
  ...
  it('holds every infinite animation still when the OS asks for less motion', () => {
    // One per row, so a full list is twenty of them at once. Asserted by finding the animated
    // markers rather than naming them, so a seventh state cannot be added and left spinning.
    ...
    for (const status of animated) { expect(query?.[0]).toContain(`.status-${status}`); }
  ...
  it('names every status the dot can be in, the quiet ones included', () => {
    // The name comes from the map, so no state can be reached that has no name — including the
    // quiet ones, which is where this started: the old idle dot had an empty title and announced
    // nothing at all. The map's own contents are pinned further down.
    expect(main).toContain("el.setAttribute('aria-label', STATUS_NAMES[status]");
    // And the three-state vocabulary this replaced stays gone.
    for (const gone of ["'Idle'", "'Running'", "'Waiting for response'"]) {
...
describe('webview: the six status markers', () => {
  ...
  it('gives the marker an accessible name — the shape is its only content', () => {
    ...
    // A name per state, not one generic word: the colour and the animation say nothing out loud,
    // so a state missing from STATUS_NAMES is a state a screen reader cannot report.
    const names = main.slice(main.indexOf('const STATUS_NAMES'));
    for (const status of SESSION_STATUSES) {
      expect(names.slice(0, names.indexOf('};')), status).toContain(`${status}:`);
    }
  });
