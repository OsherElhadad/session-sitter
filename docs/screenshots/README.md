# Screenshots, and what is still missing

The panel shots in this directory are **generated**, not hand-captured: `make screenshots` renders
the real `src/webview/` files in headless chromium and writes every PNG here. See
[`tools/screenshots/README.md`](../../tools/screenshots/README.md). Do not edit them by hand, and do
not commit a shot the harness did not produce — regenerate instead, so a screenshot can never drift
from the shipped panel. Their fixtures are entirely invented, which is deliberate: these images are
public.

What the harness **cannot** produce is anything needing a real IDE on a real screen — chiefly the
demo GIF, and any shot of the panel docked inside VS Code with its surroundings. Those are listed
below, precisely enough to execute in about ten minutes on a machine with a display.
...
## Already generated by the harness

`make screenshots` produces these, in both themes, and the README already uses them — nothing to do:
`panel`, `panel-wide`, `panel-needs-you`, `hover-preview`, `sort-menu`, `toolbar-menu`, `activity`.
They cover the "four sources in one panel" claim, the needs-you ordering, the hover preview, both
menus, and one activity card per light.

## Still needs a real screen

These are the ones the harness cannot reach, because they need a running IDE, a second host, or a
phone.

| File | What to capture | Referenced from |
|---|---|---|
| ~~`panel-worklist.png`~~ | superseded by the harness's `panel-{dark,light}.png`. | The worklist with **all four sources visible at once** — at least one Claude Code, one IBM Bob, one Codex and one VS Code Chat row — showing three different status dots (running / waiting on the agent / idle), each row's source badge, and two different workspace pill colours. This is the one screenshot that carries the whole "four sources in one panel" claim. | `README.md` §*How it finds your sessions* |
...
| ~~`hover-preview.png`~~ | superseded by the harness's `hover-preview-{dark,light}.png`, though a version with a real cursor would still read better. | `README.md` §*First run* |
| ~~`sort-menu.png`~~ | superseded by the harness's `sort-menu-{dark,light}.png`. | `docs/CONFIGURATION.md` §*Sorting the session list* |
...
|---|---|
| `marketplace-1.png` | the worklist again, but uncropped at 1280 × 800 with the editor visible beside the panel, so the panel is shown in context |
