...
// Compiled from src/telegram/lease.ts by scripts/build-plugin-lib.js (`make plugin`).
 ...
 */
function leaseIsFree(lease, now, isAlive) {
    if (lease === null) {
        return true;
    }
...
class ReaderLease {
    ...
    record() {
        return { pid: this.pid, expiresAt: this.now() + exports.LEASE_TTL_MS, host: this.host };
    }
    /**
    ...
    async tryAcquire() {
        ...
        if (!leaseIsFree(current, this.now(), this.isAlive)) {
            if (this.held) {
                this.log(`telegram lease: lost to pid ${current?.pid}`);
            }
            this.held = false;
            return false;
        }
        ...
        if (current !== null) {
            ...
            catch { /* another window got there */ }
        }
        this.held = await this.claim();
        if (this.held) {
            this.log(`telegram lease: acquired by pid ${this.pid}`);
        }
        return this.held;
    }
...
exports.ReaderLease = ReaderLease;
