...
// Compiled from src/cli/export.ts by scripts/build-plugin-lib.js (`make plugin`).
...
Options:
  ...
  --limit N            keep at most the newest N records (default ${exports.DEFAULT_LIMIT})
  -h, --help           show this help

The team scope drops, rather than blanks:
  ${exports.NEVER_SHIPPED.join(', ')}
...
function projectTeam(record, key) {
    ...
    return {
        ...
        inputFingerprint: record.call ? (0, trail_1.fingerprint)(record.call.tool_name, record.call.input) : null,
    };
}
/**
...
function hmacKey(env = process.env) {
    const file = path.join((0, paths_1.dataDir)(env), '.export-key');
    try {
 ...
 */
function rungOf(record) {
    const { actor, decision } = record;
    if (actor === 'deterministic') {
        return decision === 'allow' ? '1' : '5';
    }
    if (actor === 'correction') {
        return decision === 'allow' ? '2' : "2'";
    }
    if (actor === 'policy') {
        return decision === 'allow' ? '4' : '3';
    }
    if (actor === 'model') {
        return '6';
    }
...
<div class="band">
  <dl>
    ...
    <dt>window</dt><dd class="mono">${instants.length === 0 ? NOT_RECORDED
        : `${escapeHtml(instants[0])} → ${escapeHtml(instants[instants.length - 1])}`}</dd>
    <dt>records</dt><dd>${records.length}${truncated ? ` — <strong>truncated</strong> by --limit from ${meta.total} in the window` : ''}</dd>
    ...
    <dt>produced by</dt><dd class="mono">session-sitter ${escapeHtml(buildInfo_1.BUILD_VERSION)} on ${escapeHtml(meta.host)}</dd>
  </dl>
...
function parseScope(raw) {
    if (raw === undefined || raw === 'local') {
        return 'local';
    }
    if (raw === 'team') {
        return 'team';
    }
    throw new args_1.CliError(`--scope must be local or team, got "${raw}"`);
}
async function run(argv, io) {
    const args = (0, args_1.parseFlags)(argv, exports.EXPORT_SPEC);
    if ((0, args_1.flagBool)(args, '--help') || (0, args_1.flagBool)(args, '-h')) {
        io.out(exports.HELP);
        return 0;
    }
    const wantJsonline = (0, args_1.flagBool)(args, '--jsonline');
    const wantHtml = (0, args_1.flagBool)(args, '--html');
    if (wantJsonline === wantHtml) {
        throw new args_1.CliError('export needs exactly one of --jsonline or --html');
    }
    const scope = parseScope((0, args_1.flagString)(args, '--scope'));
    const sinceRaw = (0, args_1.flagString)(args, '--since');
    const since = sinceRaw === undefined ? null : (0, time_1.parseSince)(sinceRaw, io.now());
    const limit = (0, args_1.flagNumber)(args, '--limit') ?? exports.DEFAULT_LIMIT;
    const file = (0, paths_1.decisionsPath)();
    if (!fs.existsSync(file)) {
        throw new args_1.CliError(`no decision trail at ${file} — nothing has been governed on this machine`, 1);
    }
    const all = (0, trail_1.readJsonl)(file)
