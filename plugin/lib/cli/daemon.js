...
// Compiled from src/cli/daemon.ts by scripts/build-plugin-lib.js (`make plugin`).
...
exports.outboxBacklog = outboxBacklog;
...
exports.HELP = `session-sitter daemon — keep supervision running without an IDE
 ...
 */
async function liveIdeWindows(homedir = os.homedir()) {
    return (0, WindowRegistry_1.readLiveWindows)({ homedir });
}
function ideRefusal(windows) {
 ...
 */
function gatedChannel(inner, canRead) {
    const gated = {
 ...
 */
async function outboxBacklog(config) {
    try {
        const entries = await fs.promises.readdir((0, config_1.outboxDir)(config), { withFileTypes: true });
...
function parseInterval(args) {
    ...
    if (!Number.isFinite(n) || !Number.isInteger(n) || n < 1) {
        throw new args_1.CliError('--interval needs a whole number of seconds, 1 or more');
    }
    return n;
}
...
async function runStatus(io, json, env = process.env) {
    ...
    const lines = [
        ...
        paint(`  started   ${(0, time_1.shortStamp)(new Date(beat.startedAt))}`, 'dim'),
        paint(`  last pass ${(0, time_1.shortStamp)(new Date(beat.lastPassAt))}`, 'dim'),
        paint(`  passes    ${beat.passes}, ${beat.processed} record(s) transitioned`, 'dim'),
        paint(`  reading   ${beat.reading ? 'yes — holds the Telegram lease' : 'no — timeouts only'}`, 'dim'),
        paint(`  state dir ${beat.stateDir}`, 'dim'),
    ];
    if (state === 'stale') {
        // The distinction that matters: the process is up, and it is not doing the job.
        lines.push('', paint('The process is alive but has not finished a pass in a long time. Timeouts are NOT being '
            + 'applied.', 'red'));
    }
    if (state === 'dead') {
    ...
    if (state === 'oneshot') {
        lines.push('', paint('This was a --once run, so the process exiting is expected. Nothing is resident: timeouts are '
...
async function run(argv, io) {
    ...
    if (args.positional.length > 0) {
        throw new args_1.CliError(`daemon takes no arguments, got "${args.positional[0]}"`);
    }
    const json = (0, args_1.flagBool)(args, '--json');
    if ((0, args_1.flagBool)(args, '--status')) {
        return runStatus(io, json);
    }
    ...
    if (windows.length > 0 && !(0, args_1.flagBool)(args, '--allow-with-ide')) {
        io.err(`session-sitter daemon: ${ideRefusal(windows)}\n`);
        return 1;
    }
    const log = (msg) => {
        io.err(`${(0, time_1.shortStamp)(io.now())} ${msg}\n`);
    };
    log(`state dir ${config.stateDir}`);
    if (windows.length > 0) {
        log(`--allow-with-ide: running alongside ${windows.length} live extension host(s)`);
    }
    const lease = new lease_1.ReaderLease({
        leasePath: (0, bus_1.leasePath)(),
        pid: process.pid,
        host: os.hostname().split('.')[0],
        log,
    });
    ...
    const orchestrator = (0, factory_1.buildOrchestrator)({
        config,
        log,
        channel: gatedChannel((0, factory_1.buildChannel)(config, log), () => !leased || holdsLease),
    });
