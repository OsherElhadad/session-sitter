...
// Compiled from src/policy/corrections.ts by scripts/build-plugin-lib.js (`make plugin`).
...
exports.CORRECTION_RULES = void 0;
...
const FORCE_PUSH = {
    ...
    rewrite(command) {
        ...
        if (/--force-with-lease\b/.test(command)) {
            return null;
        }
        const long = /--force\b/;
...
exports.CORRECTION_RULES = [FORCE_PUSH, CHMOD_777];
/**
 ...
 */
function applyCorrection(toolName, toolInput) {
    if (!toolInput) {
        return null;
    }
    ...
    for (const rule of exports.CORRECTION_RULES) {
        if (rule.toolName !== toolName || !rule.test.test(command)) {
            continue;
        }
