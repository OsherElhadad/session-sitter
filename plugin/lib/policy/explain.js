...
// Compiled from src/policy/explain.ts by scripts/build-plugin-lib.js (`make plugin`).
...
exports.ExplainRefusal = void 0;
exports.explainCall = explainCall;
...
class ExplainRefusal extends Error {
    constructor(message, exitCode) {
        super(message);
        this.exitCode = exitCode;
    }
}
exports.ExplainRefusal = ExplainRefusal;
/**
 ...
 */
async function inputsFor(query, settings) {
    const rev = query.rev ?? null;
    if (rev !== null && rev !== 'current') {
        const hex = (0, compile_1.revisionHex)(rev);
        if (!/^[0-9a-f]{64}$/.test(hex)) {
            throw new ExplainRefusal(`"${rev}" is not a revision — expected 64 hex characters, or "current"`, 2);
        }
        const file = (0, compile_1.artifactPath)(`sha256:${hex}`);
        const { policy, reason } = (0, compile_1.loadPolicyFile)(file, {
            user: settings.user ?? '', project: settings.project ?? '', team: settings.team ?? '',
        });
        if (policy === null) {
            throw new ExplainRefusal(reason === 'absent'
                ? `revision ${hex.slice(0, 8)} is not retained in ${(0, compile_1.policyDir)()} — only the newest `
                ...
                : `revision ${hex.slice(0, 8)} could not be read: ${reason}. Nothing was answered from a `
                    + 'different revision instead', 1);
        }
        return {
            clauses: policy.clauses.filter(c => c.status === 'accepted').map(permissionRequest_1.clauseFromCompiled),
            compiled: policy,
            source: 'artifact',
            rev: policy.revision,
            reason: null,
    ...
    try {
        return await (0, permissionRequest_1.loadPolicyInputs)(settings);
    }
    catch (err) {
 ...
 */
function unreadableReason(settings, err) {
...
function decidingClause(clauses, citation) {
    if (citation === null) {
        return null;
    }
...
function citedFrom(inputs, citation) {
    if (inputs.compiled === null || citation === null) {
        return null;
    }
    const byCitation = new Map(inputs.compiled.clauses.map(c => [c.citation, c]));
    const clause = byCitation.get(citation);
    return clause ? (0, select_1.cite)(inputs.compiled, (0, select_1.clauseIndex)(inputs.compiled), clause.id) : null;
}
/**
 ...
 */
async function explainCall(query, settings = (0, settings_1.loadSettings)()) {
    const started = process.hrtime.bigint();
    const inputs = await inputsFor(query, settings);
    const verdict = (0, permissionRequest_1.decideDeterministically)({ tool_name: query.tool, tool_input: query.input ?? undefined }, inputs.clauses);
    const clause = decidingClause(inputs.clauses, verdict?.clause ?? null);
    const cited = citedFrom(inputs, verdict?.clause ?? null);
    let selection = null;
    if (inputs.compiled) {
        const chosen = (0, select_1.selectForPolicy)(inputs.compiled, {
            haystack: (0, session_1.haystackFor)(query.tool, query.input),
...
function renderExplain(answer) {
    const rev = answer.policy.rev === null
        ? 'no artifact'
        : `revision ${(0, compile_1.revisionHex)(answer.policy.rev).slice(0, 8)}`;
    const lines = [
    ...
    if (answer.clause !== null) {
        ...
        if (answer.sourceFile) {
            lines.push(`  ↳ source: ${answer.sourceFile}`);
        }
    ...
    if (answer.rewritten !== null) {
        lines.push('', '  The call would be rewritten to:', `  ${JSON.stringify(answer.rewritten)}`, '  That rewrite is what the practices already accept; it is not a negotiation.');
    }
...
`;
const FLAGS = ['command', 'input', 'rev', 'json', 'help'];
function valueOf(argv, name) {
    ...
    if (value === undefined) {
        throw new ExplainRefusal(`--${name} needs a value`, 2);
    }
    return value;
}
/** Parse, answer, print. The only function that knows about argv or about exit codes. */
async function runExplain(argv, io, settings = (0, settings_1.loadSettings)()) {
    try {
        if (argv.includes('-h') || argv.includes('--help')) {
            io.out(USAGE);
            return 0;
        }
        const bad = argv.find(a => a.startsWith('--') && !FLAGS.includes(a.slice(2)));
        if (bad !== undefined) {
        ...
        let input = command === null ? null : { command };
        if (raw !== null) {
            let parsed;
            try {
                ...
                throw new ExplainRefusal(`--input is not valid JSON: ${err instanceof Error ? err.message : String(err)}`, 2);
            }
