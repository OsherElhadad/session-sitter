...
// Compiled from src/supervisor/tiers.ts by scripts/build-plugin-lib.js (`make plugin`).
...
exports.greenAssessment = greenAssessment;
...
function preClassify(session) {
    ...
    for (const pat of DESTRUCTIVE) {
        if (pat.test(hay)) {
            return models_1.TrafficLight.RED;
        }
    }
    if (isSafeRead(session)) {
        return models_1.TrafficLight.GREEN;
    }
    return null;
}
...
function redAssessment(session) {
    const p = session.pendingAction;
    const action = (p ? (p.name || 'the requested action') : 'the requested action');
    return {
        traffic_light: models_1.TrafficLight.RED,
        confidence: 1.0,
...
function greenAssessment(session) {
    const p = session.pendingAction;
    const action = actionLabel(session);
    return {
        traffic_light: models_1.TrafficLight.GREEN,
        confidence: 1.0,
