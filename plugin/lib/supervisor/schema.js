...
// Compiled from src/supervisor/schema.ts by scripts/build-plugin-lib.js (`make plugin`).
...
exports.SchemaError = exports.VALID_SCOPES = exports.VALID_SEVERITIES = exports.VALID_LIGHTS = void 0;
exports.iterTopLevelObjects = iterTopLevelObjects;
exports.extractJsonObject = extractJsonObject;
exports.salvageAssessmentFromText = salvageAssessmentFromText;
exports.unclassifiedOrangeAssessment = unclassifiedOrangeAssessment;
exports.parseAndValidate = parseAndValidate;
exports.validate = validate;
const models_1 = require("./models");
exports.VALID_LIGHTS = new Set(Object.values(models_1.TrafficLight));
exports.VALID_SEVERITIES = new Set(Object.values(models_1.Severity));
exports.VALID_SCOPES = new Set(['user', 'project', 'team']);
...
function iterTopLevelObjects(text) {
    const out = [];
    let i = 0;
    const n = text.length;
    while (i < n) {
...
function extractJsonObject(raw) {
    if (raw === null || raw === undefined) {
        throw new SchemaError('output is empty');
    }
    let text = raw.trim();
 ...
 */
function salvageAssessmentFromText(raw, actionLabel = 'the requested action') {
    const text = (raw ?? '').trim();
    if (!text) {
        return null;
    }
    ...
    try {
        ...
        if (obj && typeof obj === 'object' && 'traffic_light' in obj) {
            return null;
        }
    }
    ...
    if (light === null) { // no "X light" phrasing — take the most severe light word present
        ...
        light = ['red', 'orange', 'yellow', 'green'].find(lt => present.has(lt)) ?? null;
    }
    if (light === null) {
        return null;
    }
 ...
 */
function unclassifiedOrangeAssessment(actionLabel, raw = '') {
    const detail = (raw ?? '').trim().replace(/\n/g, ' ').slice(0, 300);
    let note = 'The supervisor could not automatically classify this action (the classifier returned an '
...
function validate(data) {
    ...
    require_(typeof light === 'string' && exports.VALID_LIGHTS.has(light), `unsupported traffic_light: ${JSON.stringify(light)} (allowed: ${[...exports.VALID_LIGHTS].sort().join(', ')})`);
    const conf = d.confidence;
    require_(typeof conf === 'number' && Number.isFinite(conf), 'confidence must be a number');
    require_(conf >= 0 && conf <= 1, 'confidence must be in [0.0, 1.0]');
    for (const f of ['summary', 'agent_intent', 'user_intent', 'waiting_reason', 'recommended_action']) {
        require_(typeof d[f] === 'string', `${f} must be a string`);
    }
    require_(Array.isArray(d.issues), 'issues must be a list');
    d.issues.forEach((issue, idx) => validateIssue(issue, idx));
    ...
    validateConditionals(d, light);
    return (0, models_1.assessmentFrom)(d);
}
function validateIssue(issue, idx) {
    require_(!!issue && typeof issue === 'object' && !Array.isArray(issue), `issues[${idx}] must be an object`);
    const o = issue;
    for (const f of ['description', 'severity', 'reasoning']) {
    ...
    ev.forEach((e, j) => {
        require_(!!e && typeof e === 'object' && !Array.isArray(e), `issues[${idx}].evidence_from_session[${j}] must be an object`);
        const ref = e.reference ?? '';
        require_(typeof ref === 'string', `issues[${idx}].evidence_from_session[${j}].reference must be a string`);
    });
    const kn = o.relevant_knowledge ?? [];
    require_(Array.isArray(kn), `issues[${idx}].relevant_knowledge must be a list`);
    kn.forEach((k, j) => {
        require_(!!k && typeof k === 'object' && !Array.isArray(k), `issues[${idx}].relevant_knowledge[${j}] must be an object`);
    });
}
/** Per-traffic-light required intervention fields. */
function validateConditionals(d, light) {
    if (light === models_1.TrafficLight.YELLOW) {
        const msg = d.supervisor_message_to_agent;
        require_(typeof msg === 'string' && msg.trim() !== '', 'yellow requires a non-empty supervisor_message_to_agent');
    }
    else if (light === models_1.TrafficLight.ORANGE) {
        ...
        require_(typeof note === 'string' && note.trim() !== '', 'orange requires a non-empty human_notification');
        require_(Array.isArray(d.blocked_actions) && d.blocked_actions.length > 0, 'orange requires at least one blocked_action');
        require_('allowed_actions_while_waiting' in d && Array.isArray(d.allowed_actions_while_waiting), 'orange requires allowed_actions_while_waiting (may be empty)');
    }
    else if (light === models_1.TrafficLight.RED) {
        ...
        const note = d.human_notification;
        require_(typeof note === 'string' && note.trim() !== '', 'red requires a non-empty human_notification');
    }
    // Orange→Yellow timeout fallback shape.
    if (d.transitioned_from !== undefined && d.transitioned_from !== null) {
        require_(d.transitioned_from === models_1.TrafficLight.ORANGE, "transitioned_from must be 'orange' when present");
        require_(light === models_1.TrafficLight.YELLOW, 'a transitioned_from=orange result must have traffic_light=yellow');
        require_(d.transition_reason !== undefined && d.transition_reason !== null, 'transition_reason is required when transitioned_from is set');
        const msg = d.supervisor_message_to_agent;
        require_(typeof msg === 'string' && msg.trim() !== '', 'an orange->yellow fallback requires supervisor_message_to_agent');
    }
}
function validateOptionalStr(d, key) {
    if (key in d && d[key] !== null && d[key] !== undefined) {
        require_(typeof d[key] === 'string', `${key} must be a string or null`);
    }
}
function validateOptionalStringList(d, key) {
    if (key in d && d[key] !== null && d[key] !== undefined) {
...
function validateOptionalBool(d, key) {
    if (key in d && d[key] !== null && d[key] !== undefined) {
