...
// Compiled from src/supervisor/questions.ts by scripts/build-plugin-lib.js (`make plugin`).
...
exports.questionSpecFrom = questionSpecFrom;
exports.questionAnswerFrom = questionAnswerFrom;
exports.isQuestion = isQuestion;
exports.normalizeQuestion = normalizeQuestion;
exports.formatAnswerDeliveryText = formatAnswerDeliveryText;
 ...
 */
const QUESTION_TOOLS = new Set(['ask_followup_question', 'AskUserQuestion']);
function isQuestion(session) {
...
function normalizeQuestion(session) {
 ...
 */
function formatAnswerDeliveryText(spec, answer) {
    ...
    for (const q of keys) {
        ...
        if (chosen.length) {
            lines.push(`${q}: ${chosen.map(c => String(c)).join(', ')}`);
        }
    }
    return lines.join('\n');
}
function normalizeClaude(pa) {
    const rawQs = (pa.arguments?.questions ?? []);
